...
def threshold_image(img, visualise=False):
    ...
    gray_bin = np.zeros_like(s_bin)
    gray_bin[(mag_bin == 1) | (s_bin == 1) | (dir_bin == 1)] = 1
...
def undistort(img, mtx, dist, visualise=False):
...
def view_road_top(img, img_bin, visualise=False):
    ...
    img_top = cv2.warpPerspective(img, M, img_size, flags=cv2.INTER_LINEAR)
